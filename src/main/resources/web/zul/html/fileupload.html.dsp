...
<body onload="init()">
	...
	<div style="overflow-y:auto;overflow-x:hidden;padding-top:8px;width:100%;height:${param.max > 3 ? '100px' : ''};">
...
<c:forEach var="cnt" begin="1" end="${maxcnt}" varStatus="s">
	<tr>
		...
		<td align="center"><input class="file" type="file" id="file" size="28" name="file"/><c:if test="${unlimited && s.index == maxcnt}"><img src="${c:encodeURL('~./zul/img/add.gif')}" onclick='addUpload(this);'"/></c:if><c:if test="${unlimited && s.index < maxcnt}"><img src="${c:encodeURL('~./zul/img/delete.gif')}" onclick='deleteUpload(this);'"/></c:if></td>
	</tr>
</c:forEach>
	</table>
	</div>
	...
	<tr align="left">
		<td colspan="2" style="border: outset 1px;padding: 2px 12px;background-color:#FAFAFA;">
